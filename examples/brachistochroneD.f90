...
program brachD
  ...
  !---------------------------------------------------------------------
  ! Solve the problem, call snctrlD.
  !---------------------------------------------------------------------
...
subroutine odecon ( snStat, curPhs, nPhs, nY, nU, nP, nNodes, F, J, &
  ...
  if ( needJ > 0 ) then
     do jt = 1, nNodes
        J(1,1,jt) = 0.0
        J(1,2,jt) = sqrt((1-dvar(2,jt))/(1+dvar(nY+1,jt)**2))* &
             (1+dvar(nY+1,jt)**2) &
             *(1/2.0)/( (1-dvar(2,jt))**2)
        J(1,3,jt) = sqrt((1-dvar(2,jt))/(1+dvar(nY+1,jt)**2))* &
